...
v2 change: tuning uses gap_aware_tune (shared in tuning.py) instead of
mod.tune()'s plain best-CV-RMSE selection - every hyperparameter search in
this pipeline is now gap-aware, not just the final-tuning step (see
tuning.py's docstring for why).
"""
import argparse
import json
import time
...
def main():
    ...
    log(f"=== gap-aware tuning {args.model} ({n_trials} trials) ===")
    t0 = time.time()
