"""Spectral index feature engineering, plus the site/weather feature-group
axis definitions shared across the whole v2 pipeline.
...
SITE_COLS / WEATHER_PREFIXES / weather_cols() are the single source of
truth for the site-vs-weather-vs-optical feature-group axis (v1 duplicated
SITE_COLS as an independent literal in run_feature_group_model.py,
run_backward_elimination.py, and run_shap_cv.py, which is exactly the kind
of drift risk that matters when a new feature - shoreline_flag - needs to
land in all of them consistently). shoreline_flag is grouped with SITE_COLS
since it's another static per-site/catchment attribute already living in
site_characteristics.feather, not a new axis of its own.
"""
import numpy as np
import pandas as pd
...
def weather_cols(df: pd.DataFrame) -> list:
