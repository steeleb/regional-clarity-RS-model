# The geometry-QA floor (mandatory, see 02_filter_ablation.R) caps final
# training rows at ~6,800-6,900 even with metadata/cloud QA fully removed -
# short of the 10k target. So outlier/QA loosening alone can't get there;
# this script tests the other lever available: widening the 5-day match
# window, combined with a looser (not removed) cloud-cover threshold, since
# cloud cover was the single biggest legitimate-QA cost in the ablation
# (~18% of matches vs ~6% for metadata QA).
...
scene_metadata_all <- purrr::map(scene_metadata_paths, read_csv, show_col_types = FALSE) %>% bind_rows()
...
siteSR_geom <- open_dataset(unified_data, format = "feather") %>%
  ...
  collect()
log("rows: %s", format(nrow(siteSR_geom), big.mark = ","))

sdd_new <- read_feather(file.path(aquamatch_dir, "sdd_quality_filter.feather"), col_select = -is_inlier)
# keep SDD-side QA at current (new) settings throughout - it's ~free per the
# prior ablation (0.3% cost) and is a legitimate, well-justified filter set
...
## ---- cloud-cover threshold variants (all geometry-QA'd) ----
sr_cloud20 <- siteSR_geom %>% filter(sat_id %in% (scene_metadata_all %>% filter(CLOUD_COVER < 20) %>% pull(sat_id)))
sr_cloud40 <- siteSR_geom %>% filter(sat_id %in% (scene_metadata_all %>% filter(CLOUD_COVER < 40) %>% pull(sat_id)))
sr_cloud60 <- siteSR_geom %>% filter(sat_id %in% (scene_metadata_all %>% filter(CLOUD_COVER < 60) %>% pull(sat_id)))
# metadata QA (pCount thresholds) applied on top of each, since it's cheap and legitimate
add_meta <- function(df) df %>% filter(pCount_dswe1 / pCount_dswe_gt0 > 0.5, pCount_dswe1 > 8, prop_clouds == 0)

configs <- list(
  list(label = "geom+meta+cloud20_w5",  df = add_meta(sr_cloud20), w = 5),
  list(label = "geom+meta+cloud40_w5",  df = add_meta(sr_cloud40), w = 5),
  list(label = "geom+meta+cloud60_w5",  df = add_meta(sr_cloud60), w = 5),
  list(label = "geom+meta+cloud40_w7",  df = add_meta(sr_cloud40), w = 7),
  list(label = "geom+meta+cloud40_w10", df = add_meta(sr_cloud40), w = 10),
  list(label = "geom+meta+cloud60_w7",  df = add_meta(sr_cloud60), w = 7),
  list(label = "geom+meta+cloud60_w10", df = add_meta(sr_cloud60), w = 10)
)
...
readr::write_csv(grid_tbl, file.path(out_dir, "window_cloud_grid_results.csv"))
log("GRID RESULTS:")
print(grid_tbl)
log("done")
