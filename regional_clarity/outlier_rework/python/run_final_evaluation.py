...
decision that got here - feature set (intersection-16, backward
elimination), hyperparameters (gap-aware tuning), and SDD-weighting
(established earlier: helps XGBoost/LightGBM, hurts the capped NN) - was
already made from CV (partitions 1-4) alone. This script only reports the
outcome of those decisions on the untouched test partition; it makes no
new decisions from test performance.
...
def main():
    ...
    for name, mod in MODELS.items():
        ...
        log(f"{name} features (n={len(INTERSECTION16)}): {INTERSECTION16}")
        ...
        test_pred = mod.predict_ensemble(fold_models, test[INTERSECTION16])
        ...
        summary[name] = dict(weighted=USE_WEIGHTING[name], params={k: str(v) for k, v in params.items()},
                              features=INTERSECTION16, cv_metrics=cv_metrics, test_metrics=test_metrics)
