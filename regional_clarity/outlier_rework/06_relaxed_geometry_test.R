# Test: 5-day window (locked, per user), geometry QA relaxed to ONLY
# number_int_wb == 1 (drop the shoreline-proximity flags). The earlier
# blowup came from removing number_int_wb entirely (letting one WQP site
# fan out across many ambiguously-assigned, multi-waterbody siteSR
# candidates) - shoreline flags are a per-site contamination-risk QA, not a
# multiplicity control, so dropping just those should be safe. Testing
# with the same disk-safety monitoring regardless.
...
sites_geom_relaxed <- sites_raw %>%
  filter(number_int_wb == 1)   # drop shoreline flags only

log("sites: full geometry QA = %s, relaxed (single-wb only) = %s",
    format(nrow(sites_geom_full), big.mark=","), format(nrow(sites_geom_relaxed), big.mark=","))
...
siteSR_relaxed <- open_dataset(unified_data, format = "feather") %>%
  filter(huc2 %in% regional_huc2, siteSR_id %in% sites_geom_relaxed$siteSR_id) %>%
  collect()
log("candidate rows (relaxed geometry, no metadata/cloud QA yet): %s", format(nrow(siteSR_relaxed), big.mark=","))
...
sr_relaxed_meta <- add_meta(siteSR_relaxed)
log("after metadata QA: %s rows", format(nrow(sr_relaxed_meta), big.mark=","))

result <- run_config("relaxedgeom+meta_w5", sr_relaxed_meta, sdd_new, 5)
print(result)
readr::write_csv(result, file.path(out_dir, "relaxed_geometry_test_results.csv"))
log("done")
