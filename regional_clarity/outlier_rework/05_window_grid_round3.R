...
sr_meta_nocloud <- add_meta(siteSR_geom)
configs <- list(
  list(label = "meta+nocloud_w16", df = sr_meta_nocloud, w = 16),
  list(label = "meta+nocloud_w18", df = sr_meta_nocloud, w = 18)
)
results <- list()
for (cfg in configs) {
...
readr::write_csv(grid_tbl, file.path(out_dir, "window_grid_round3_results.csv"))
log("ROUND 3 RESULTS:"); print(grid_tbl); log("done")
